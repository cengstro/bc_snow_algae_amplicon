...
```{r rotated-stacked-bar, echo=F, fig.width=7, fig.height=5}

# specify factor order, so Chlainomonas always on same side, etc
species_colors <- c("Chlainomonas A"="#984ea3","Chlainomonas rubra"="#984ea3", #purple
                "Sanguina B"="#ff534a", "Sanguina nivaloides"="#ff534a", #red
                # Chloromonas: pastel greens / blues / yellows
                "Chloromonas krienitzii"="#ff7f00","Chloromonas muramotoi C"="#87b1d4", "Chloromonas D"="#94cf92", "Chloromonas F"="#fdfd96", 
                ...
                # greyish light green
                "Trebouxiophyceae G"="#173416", "Trebouxiophyceae"="#173416", # dark green, stand out
                "Unknown E"="grey60", "Green cell"="#4aaf53", "Other"="grey30") #greys
...
stacked_bar <- ggplot(combined, aes(x=alias %>% fct_reorder(elev_m), y=rel_abund)) +
  ...
  coord_flip() +
  facet_grid(cols = vars(marker %>% fct_relevel(c("Cell morphology","18S","rbcL"))), 
             scales="free_y", space="free_y") +
  theme_classic() +
...
stacked bar legend
```{r}
legend <- stacked_bar %>% cowplot::get_legend()
grid.newpage()
grid.draw(legend)
```
