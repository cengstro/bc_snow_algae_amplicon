...
otu_names %>% distinct(otu_name)

# order OTUs
otu_ord <- c("Chlainomonas A","Sanguina B", "Chloromonas spp. C","Chloromonas krienitzii D","Chloromonas muramotoi E","Chloromonas F", "Algae G","Chloromonas H", "Trebouxiophyceae I")

barplot <- ggplot(plot_data, aes(x=alias %>% fct_reorder(elev_m), y=rel_abund)) +
...
combined_fig_3 <- grid.arrange(uf_plot, barplot, nrow=2)
```
...
```{r output, include=F}
ggsave(filename = here("manuscript/figs/3_community/combined_fig_3.pdf"), plot=combined_fig_3, height=10, width=7)
