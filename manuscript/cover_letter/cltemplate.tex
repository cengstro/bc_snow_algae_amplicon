...
\setromanfont{Liberation Serif}         % CE reset using available fonts
\setsansfont[BoldFont={Liberation Sans Bold}]{Liberation Sans}
...
\newcommand{\header}[9]{
    ...
    \parbox[b][0.625in][t]{6.5in}{
        ...
        \parbox[b][0.625in][t]{3.74in}
            {\href{http://www.sfu.ca}
            {\includegraphics[height=0.6in]{./sfu_long.pdf}}}\hfill
        \parbox[b][0.625in][t]{2in}{
            \setlength{\baselineskip}{9pt}
            \fontsize{7pt}{7pt}{
                \fontspec{Liberation Sans}
                \textbf{#8}\\
                {#9}\vspace{0pt}\\  %CE remove space btwn dept and address
                #1\\
                #2, #3 #4\\
                Phone: #5\\
                %Fax: #6\\    % CE no fax
