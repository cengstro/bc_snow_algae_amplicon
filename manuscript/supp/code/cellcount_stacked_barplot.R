...
cellct_plot <- ggplot(cellct, aes(x=alias %>% fct_reorder(elev_m), 
    ...
    theme_classic() +
    labs(x="Sample ID", y="Relative abundance", fill="Cell morphology")+
    scale_fill_manual(values = colors) +
    theme(text = element_text(size=8),
          axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5))  
cellct_plot

ggsave(filename = here::here("manuscript/supp/output/fig_S4_cellct.tiff"), width = 300, height = 100, units="mm", dpi=300)
